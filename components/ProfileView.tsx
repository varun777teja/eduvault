...
const ProfileView: React.FC<ProfileViewProps> = ({ documents, onLogout }) => {
  const [stats, setStats] = useState({ minutes: 0, ai_hits: 0, streak: 0 });
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      const localStats = localStorage.getItem('eduvault_study_stats');
      if (localStats) {
        rawData = JSON.parse(localStats);
      } else {
        // Fallback or Supabase logic if configured
        const { data } = await supabase.from('study_sessions').select('*');
        if (data) {
          data.forEach((row: any) => rawData[row.date] = row.minutes);
        }
      }

      // Calculate Totals & Streak
      Object.values(rawData).forEach(mins => totalMinutes += mins);

      // Streak Logic: Check backwards from today
      let checkDate = new Date(today);
      while (true) {
        const dateStr = checkDate.toISOString().split('T')[0];
        if (rawData[dateStr] && rawData[dateStr] > 0) {
          currentStreak++;
          checkDate.setDate(checkDate.getDate() - 1);
        } else {
          // Allow missing today if it's early, otherwise break
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        last7Days.push({
          ...
          minutes: rawData[dateStr] || 0
        });
      }

      setStats({
        minutes: totalMinutes,
        ai_hits: parseInt(localStorage.getItem('eduvault_ai_hits') || '0'), // Assuming App.tsx or Chat tracks this too
  ...
  const achievements = [
    ...
    { title: "Quick Learner", icon: Zap, desc: "7-day study streak", color: "text-emerald-500", bg: "bg-emerald-50", unlocked: stats.streak >= 7 },
  ];
            ...
            <div className="max-w-md mx-auto md:mx-0">
              <div className="flex justify-between items-end mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black text-indigo-600 uppercase tracking-widest">Mastery Level {Math.floor(stats.minutes / 60) + 1}</span>
                </div>
                <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{stats.minutes} / {(Math.floor(stats.minutes / 1000) + 1) * 1000} XP</span>
              </div>
              <div className="h-4 w-full bg-slate-100 rounded-full overflow-hidden p-1 shadow-inner">
                <div className="h-full bg-gradient-to-r from-indigo-600 to-purple-600 rounded-full transition-all duration-1000 shadow-[0_0_12px_rgba(79,70,229,0.3)]" style={{ width: `${(stats.minutes % 1000) / 10}%` }}></div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          <div className="bg-white border border-slate-200 p-10 rounded-[4rem] shadow-sm">
              ...
              {weeklyData.map((d, i) => (
                <div key={i} className="flex flex-col items-center gap-2 flex-1 group">
                  <div
                    ...
                    style={{ height: `${Math.min((d.minutes / 60) * 100, 100)}%`, minHeight: '4px' }}
                  >
                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] py-1 px-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                      {d.minutes}m
                    </div>
                  </div>
                  <span className="text-[9px] font-black text-slate-300 uppercase">{d.day}</span>
                </div>
              ))}
            </div>
            <div className="flex items-center gap-2 mt-6 pt-6 border-t border-slate-100">
              ...
              <div>
                <p className="text-xs font-black text-slate-900">{stats.streak} Day Streak</p>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Keep it up!</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
              ...
              <div className="text-center md:text-left">
                ...
                <p className="text-base text-indigo-100 leading-relaxed max-w-xl opacity-90">
                  Your focus sessions in <span className="font-black text-white underline decoration-white/30 underline-offset-4">{userMetadata?.branch || "Engineering"}</span> are now being tracked live. Consistency is key to mastery!
                </p>
              </div>
            </div>
